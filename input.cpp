...
LPDIRECTINPUTDEVICE8	g_pDIDevMouse = NULL;               //IDirectInputDevice8インターフェースへのポインタ(Mouse)
...
HRESULT InitMouse(HINSTANCE hInst, HWND hWnd) {
	...
	hr = g_pDIDevMouse->SetDataFormat(&c_dfDIMouse);
	if (FAILED(hr))
	{
		return hr;
	}
...
bool ReadMouse()
{
	...
	result = g_pDIDevMouse->GetDeviceState(sizeof(DIMOUSESTATE), (LPVOID)&m_mouseState);
	if (FAILED(result))
	{
